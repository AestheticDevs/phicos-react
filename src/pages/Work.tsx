    ...
    <>
      <section className="grid grid-cols-2 min-h-[600px] container max-w-7xl">
        ...
        <div className="my-auto">
          ...
          <p className="leading-7 [&:not(:first-child)]:mt-6 text-slate-700 mb-6 text-xl">
            Call us up and we will line up our best design to promote your
            products. We create impressive professional media and precise
          ...
          <TabsList className="gap-8 mb-2 justify-start w-full">
            {tab.map((item) => (
              <TabsTrigger
                key={item.key}
                value={item.key}
                className="data-[state=active]:bg-background data-[state=active]:text-slate-900 py-3 rounded px-6 data-[state=active]:border-b border-gray-300"
              >
                {item.label}
              </TabsTrigger>
            ))}
